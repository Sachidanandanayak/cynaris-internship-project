...
@section('title', 'Post a New Job Opening')
...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <nav class="flex items-center gap-2 text-xs text-gray-500 font-medium">
        ...
        <span>&rsaquo;</span>
        <span class="text-gray-800 font-semibold">Post a Job</span>
    </nav>
    ...
    <div class="bg-white rounded-2xl border border-gray-200 p-6 sm:p-10 shadow-sm">
        <div class="border-b border-gray-100 pb-6 mb-8">
            <span class="text-xs font-bold uppercase tracking-wider text-indigo-600">Recruiter Portal</span>
            <h1 class="text-2xl sm:text-3xl font-black text-gray-900 mt-1">
                Publish a New Job Opening
            </h1>
            <p class="text-xs text-gray-500 mt-1">
                Provide comprehensive information to attract top engineering talent across our network.
            </p>
        </div>
        ...
        <form action="{{ route('jobs.store') }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text"
                           ...
                           value="{{ old('title') }}"
                           required
                           placeholder="e.g. Senior Laravel Engineer, Full Stack PHP Architect..."
                ...
                <div>
                    ...
                    <input type="text"
                           ...
                           value="{{ old('company') }}"
                           required
                           placeholder="e.g. Cynaris Systems Inc."
                ...
                <div>
                    ...
                    <select name="employment_type" id="employment_type" required class="w-full px-4 py-2.5 bg-gray-50 border @error('employment_type') border-red-500 @else border-gray-300 @enderror rounded-lg text-sm text-gray-900 focus:ring-2 focus:ring-indigo-500 outline-none transition">
                        @foreach (['Full-time', 'Part-time', 'Contract', 'Remote', 'Internship'] as $type)
                            <option value="{{ $type }}" {{ old('employment_type', 'Full-time') === $type ? 'selected' : '' }}>
                                {{ $type }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    <label for="salary_range" class="block text-xs font-bold uppercase tracking-wider text-gray-700 mb-1.5">
                        Compensation / Salary Range <span class="text-gray-400 font-normal lowercase">(optional)</span>
                    </label>
                    <input type="text"
                           name="salary_range"
                           id="salary_range"
                           value="{{ old('salary_range') }}"
                           placeholder="e.g. ₹15,00,000 - ₹22,00,000 / yr or $90,000 - $120,000"
                ...
                <div>
                    <label for="application_deadline" class="block text-xs font-bold uppercase tracking-wider text-gray-700 mb-1.5">
                        Application Deadline <span class="text-gray-400 font-normal lowercase">(optional)</span>
                    </label>
                    <input type="date"
                ...
                <div>
                    ...
                    <select name="status" id="status" class="w-full px-4 py-2.5 bg-gray-50 border @error('status') border-red-500 @else border-gray-300 @enderror rounded-lg text-sm text-gray-900 focus:ring-2 focus:ring-indigo-500 outline-none transition">
                        <option value="active" {{ old('status', 'active') === 'active' ? 'selected' : '' }}>Active (Accepting Applications)</option>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description"
                              ...
                              rows="6"
                              required
                              placeholder="Detail the day-to-day responsibilities, mission, and scope of this position..."
                              class="w-full px-4 py-2.5 bg-gray-50 border @error('description') border-red-500 @else border-gray-300 @enderror rounded-lg text-sm text-gray-900 focus:ring-2 focus:ring-indigo-500 outline-none leading-relaxed">{{ old('description') }}</textarea>
                    @error('description')
                        <span class="text-xs text-red-600 mt-1 block">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="requirements"
                              ...
                              rows="5"
                              required
                              placeholder="List technical stack prerequisites, years of experience, educational requirements..."
                              class="w-full px-4 py-2.5 bg-gray-50 border @error('requirements') border-red-500 @else border-gray-300 @enderror rounded-lg text-sm text-gray-900 focus:ring-2 focus:ring-indigo-500 outline-none leading-relaxed">{{ old('requirements') }}</textarea>
                    @error('requirements')
                        <span class="text-xs text-red-600 mt-1 block">{{ $message }}</span>
                    @enderror
                </div>
            ...
            <div class="pt-6 border-t border-gray-100 flex items-center justify-end gap-3">
                <a href="{{ route('jobs.index') }}" class="px-5 py-2.5 border border-gray-300 rounded-lg text-sm font-semibold text-gray-700 hover:bg-gray-50 transition">
                    Cancel
                </a>
